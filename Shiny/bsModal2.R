# Adapted from https://github.com/ebailey78/shinyBS/blob/shinyBS3/R/bsModal.R
# because I am not sure how to load bsModal from GitHub instead of the CRAN version
# normally available on shinyapps.io. I need this to override the close.button, etc.
# This doesn't work, apparently because it references package globals.
...
#'@template footer
#'@export
bsModal2 <- function(id, title, trigger, ..., size, footer = NULL, close.button = TRUE, width = NULL) {
if(!missing(size)) {
if(size == "large") {
size = "modal-lg"
} else if(size == "small") {
